...
(function (window, document, $) {
  ...
  // Donut chart
  // -----------
  let CurrentBalanceDonutChart = new Chartist.Pie(
  ...
  TotalTransactionLine.on('created', (data) => {
  ...
  // User Statics
  let UserStatisticsBarChart = new Chartist.Bar(
  ...
  UserStatisticsBarChart.on('draw', (data) => {
    if (data.type === "bar") {
      data.element.attr({
  ...
  UserStatisticsBarChart.on('created', (data) => {
    var defs = data.svg.querySelector("defs") || data.svg.elem("defs")
    defs
      .elem("linearGradient", {
        id: "barGradient1",
  ...
  // Conversion Ratio
  let ConversionRatioBarChart = new Chartist.Bar(
  ...
  ConversionRatioBarChart.on('draw', (data) => {
    if (data.type === "bar") {
      data.element.attr({
  ...
  ConversionRatioBarChart.on('created', (data) => {
    var defs = data.svg.querySelector("defs") || data.svg.elem("defs")
    defs
      .elem("linearGradient", {
        id: "barGradient1",
  ...
  let SLlabels = ['January', 'February', 'March', 'April', 'May', 'June'];

  let LineSL3ctx = document
    .getElementById('custom-line-chart-sample-three')
    .getContext('2d');

  let gradientStroke = LineSL3ctx.createLinearGradient(500, 0, 0, 200);
  gradientStroke.addColorStop(0, '#8133ff');
  gradientStroke.addColorStop(1, '#ff4bac');

  let gradientFill = LineSL3ctx.createLinearGradient(500, 0, 0, 200);
  gradientFill.addColorStop(0, '#8133ff');
  gradientFill.addColorStop(1, '#ff4bac');

  let SL3Chart = new Chart(LineSL3ctx, {
    type: 'line',
    data: {
      labels: SLlabels,
      datasets: [
  ...
  $('.logo-wrapper .navbar-toggler').on('click', () => {
    setTimeout(function () {
